...
class Label(Detail):
    __slots__ = 'color', 'font_type', 'font_size', 'static_txt', 'dynamic_txt', 'render'
    ...
    def __init__(self, x, y, color, font_type, font_size,  **kwargs):
        ...
        if 'static_txt' in kwargs:
            ...
            self.render, (width, height) = TextManager().get_render(self.static_txt, self.color, self.font_type, self.font_size)
            kwargs.pop('static_txt')
        if 'dynamic_txt' in kwargs:
            self.static_txt = None
            self.dynamic_txt = kwargs['dynamic_txt']
            self.render, (width, height) = TextManager().get_default_render()
    ...
    def get_surface(self):
        if self.static_txt is not None:
            ...
            self.render, (self.width, self.height) = TextManager().get_render(self.dynamic_txt(), self.color, self.font_type, self.font_size)
            return self.render
